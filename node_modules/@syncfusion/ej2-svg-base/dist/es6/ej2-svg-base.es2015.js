...
let Tooltip = class Tooltip extends Component {
    ...
    createTemplate(areaBounds, location) {
        ...
        if (firstElement) {
            if (typeof firstElement.remove === 'function') {
                firstElement.remove();
            }
            else {
        ...
        if (!argsData.cancel) {
            ...
            let templateElement = Array.prototype.slice.call(this.templateFn(this.data));
            let len = templateElement.length;
            for (let i = 0; i < len; i++) {
                elem.appendChild(templateElement[i]);
            }
