...
/** @hidden */
const tapHoldReady = 'tap-hold-ready';
...
/** @hidden */
const CLONE_ELEMENT_CLASS = 'e-event-clone';
/** @hidden */
    ...
    tapHoldHandler(e) {
        ...
        if (!isNullOrUndefined(target)) {
            if (this.parent.isAdaptive) {
                ...
                this.parent.notify(tapHoldReady, { event });
            }
            return;
        }
...
class EventBase {
    ...
    wireAppointmentEvents(element, isAllDay = false) {
    ...
    renderResizeHandler(element, spanEvent) {
    ...
    getEventMaxID(resourceId) {
        ...
        if (idType === 'number') {
            let maxId = Math.max.apply(Math, this.parent.eventsData.map((event) => event[this.parent.eventFields.id]));
    ...
    generateOccurrence(event, viewDate) {
        ...
        let dates = generate(startDate, eventRule, event[fields.recurrenceException], this.parent.firstDayOfWeek, undefined, currentViewDate);
        ...
        if (isNullOrUndefined(action)) {
            ...
            switch (action) {
                case 'EditOccurrence':
                    ...
                    let query = new Query().where('Guid', 'equal', data.Guid);
...
class QuickPopups {
    ...
    tapHoldEventPopup(args) {
        let target = closest(args.event.target, '.' + APPOINTMENT_CLASS);
        this.isMultipleEventSelect = false;
    ...
    cellClick(args) {
        ...
        let target = closest(args.event.target, '.' + WORK_CELLS_CLASS + ',.' + ALLDAY_CELLS_CLASS + ',.' +
            HEADER_CELLS_CLASS);
    ...
    eventClick(events) {
        ...
        if (this.parent.isAdaptive && this.isMultipleEventSelect) {
            ...
            let readonly = this.parent.activeViewOptions.readonly;
    ...
    moreEventClick(data, endDate, groupIndex) {
        ...
        if (this.parent.currentView.indexOf('Timeline') !== -1) {
            ...
            let index = (isNullOrUndefined(gIndex)) ? 0 : parseInt(gIndex, 10);
...
class EventTooltip {
    ...
    onBeforeRender(args) {
        ...
        if (args.target.classList.contains(RESOURCE_CELLS_CLASS) && this.parent.activeViewOptions.group.resources.length > 0) {
            ...
            let resCollection = this.parent.resourceBase.lastResourceLevel[index];
            let data = {
...
let RecurrenceEditor = class RecurrenceEditor extends Component {
    ...
    destroyComponents() {
        ...
        this.dayButtons.forEach((element) => element.destroy());
        this.dayButtons = [];
        this.monthButtons.forEach((element) => element.destroy());
        this.monthButtons = [];
    }
    resetFields() {
...
class EventWindow {
    ...
    getDefaultEventWindowContent() {
        ...
        let titleDiv = this.renderTextBox(SUBJECT_CLASS);
        let locationDiv = this.renderTextBox(LOCATION_CLASS);
        ...
        let timezoneDiv = this.renderCheckBox(TIME_ZONE_CLASS);
        ...
        let descriptionDiv = this.renderTextBox(DESCRIPTION_CLASS);
    ...
    renderMultiSelect(index) {
        let resourceData = this.parent.resourceBase.resourceCollection[index];
    ...
    showDetails(eventData) {
        ...
        for (let curElement of formelement) {
            let columnName = curElement.name;
            let isCustomElement = (curElement.classList.contains('e-multiselect') ||
    ...
    getColumnName(element) {
        let isDropDownList = element.classList.contains('e-dropdownlist');
        let inputCollection = isDropDownList ? [].slice.call(element.parentElement.children) :
            [].slice.call(element.parentElement.parentElement.children);
        let collection = inputCollection.filter((item) => item.name);
    ...
    getObjectFromFormData(className) {
        ...
        for (let currentElement of formElement) {
            let columnName = currentElement.name;
            if (!isNullOrUndefined(columnName) && columnName !== '') {
    ...
    editOccurrenceValidation(eventId, eventObj) {
        let recurColl = this.parent.getOccurrencesByID(eventId);
        let index = this.getRecurrenceIndex(recurColl, this.eventData);
...
class ResourceBase {
    ...
    setResourceValues(eventObj, isCrud) {
...
let Schedule = class Schedule extends Component {
    ...
    onPropertyChanged(newProp, oldProp) {
        ...
        let requireScheduleRefresh = false;
        for (let prop of Object.keys(newProp)) {
            switch (prop) {
                case 'width':
                case 'height':
    ...
    onGroupSettingsPropertyChanged(newProp, oldProp) {
        for (let prop of Object.keys(newProp)) {
    ...
    getCellDetails(td) {
        ...
        let endTime;
        let duration = 1;
        let endDateFromColSpan = this.activeView.isTimelineView() && !isNullOrUndefined(td.getAttribute('colSpan'));
        ...
        let groupIndex = td.getAttribute('data-group-index');
        if (!isNullOrUndefined(groupIndex)) {
...
class ActionBase {
    ...
    autoScrollValidation(e) {
        ...
        this.scrollEdges = { left: false, right: false, top: false, bottom: false };
        let viewBoundaries = this.parent.element.querySelector('.' + CONTENT_WRAP_CLASS).getBoundingClientRect();
...
class Resize extends ActionBase {
    ...
    getTopBottomStyles(e, isTop) {
        ...
        let clnHeight = isTop ? this.actionObj.element.offsetHeight + (this.actionObj.Y - pages.pageY) :
            this.actionObj.element.offsetHeight + (pages.pageY - this.actionObj.Y);
        let clnTop = isTop ? this.actionObj.element.offsetTop - (this.actionObj.Y - pages.pageY) : this.actionObj.clone.offsetTop;
    ...
    getLeftRightStyles(e, isLeft) {
        ...
        let pageWidth = isLeft ? (this.actionObj.X - pages.pageX) : (pages.pageX - this.actionObj.X);
        ...
        if (this.parent.enableRtl) {
            ...
            let offsetLeft = isLeft ? this.actionObj.element.offsetLeft - (this.actionObj.X - pages.pageX) :
...
class DragAndDrop extends ActionBase {
    wireDragEvent(element, isAllDay) {
        let dragAreaTarget = isAllDay ? DATE_HEADER_WRAP_CLASS : CONTENT_WRAP_CLASS;
    ...
    calculateVerticalTime(e) {
        ...
        let offsetTop = Math.floor(this.actionObj.clone.offsetTop / this.actionObj.cellHeight) * this.actionObj.cellHeight;
        ...
        let diffInMinutes = this.actionObj.clone.offsetTop - offsetTop;
        ...
        let index = closest(e.target, 'td').cellIndex;
    ...
    swapDragging(e) {
        let colIndex = closest(e.target, 'td').cellIndex;
        let setDragArea = (isAllDay) => {
            let dragAreaTarget = isAllDay ? DATE_HEADER_WRAP_CLASS : CONTENT_WRAP_CLASS;
            let dragObj = this.actionObj.element.ej2_instances[0];
            dragObj.dragArea = this.parent.element.querySelector('.' + dragAreaTarget);
            dragObj.dataBind();
        };
    ...
    calculateVerticalDate(e) {
        ...
        let timeString = td.getAttribute('data-date') || eventObj[this.parent.eventFields.startTime].getTime().toString();
    ...
    calculateTimelineTime(e) {
        ...
        if (this.parent.enableRtl) {
            let rightOffset = Math.abs(parseInt(this.actionObj.clone.style.left, 10)) - this.actionObj.clone.offsetWidth;
    ...
    calculateTimelineDate(e) {
        ...
        if (this.parent.enableRtl) {
            ...
            let leftOffset = parseInt(this.actionObj.clone.style.left, 10);
    ...
    calculateResourceGroupingPosition(e) {
        ...
        let rowIndex = Math.floor(Math.floor((this.actionObj.Y + dragArea.scrollTop) -
...
class VerticalEvent extends EventBase {
    ...
    renderAppointments() {
        let wrapperElements = [].slice.call(this.parent.element.querySelectorAll('.' + APPOINTMENT_CLASS + ',.' + ROW_COUNT_WRAPPER_CLASS));
    ...
    renderNormalEvents(eventObj, dayIndex, resource, dayCount) {
        ...
        let isHourRange = eEnd.getTime() > schedule.startHour.getTime() && eStart.getTime() < schedule.endHour.getTime();
        let isSameRange = schedule.startHour.getTime() <= eStart.getTime() &&
        ...
        if (eStart <= eEnd && (isHourRange || isSameRange)) {
            let appHeight = (eEnd.getTime() - eStart.getTime()) / (60 * 1000) * (this.cellHeight * this.slotCount) / this.interval;
...
class MonthEvent extends EventBase {
    ...
    renderEventElement(event, appointmentElement, cellTd) {
        let args = { data: event, element: appointmentElement, cancel: false };
        this.parent.trigger(eventRendered, args);
        if (args.cancel) {
...
class VerticalView extends ViewBase {
    ...
    getDateSlots(renderDates, workDays, workStartHour = this.parent.workHours.start, workEndHour = this.parent.workHours.end) {
        let dateCol = [];
        let start = this.parent.globalize.parseDate(workStartHour, { skeleton: 'Hm' });
        let end = this.parent.globalize.parseDate(workEndHour, { skeleton: 'Hm' });
    ...
    getCurrentTimeIndicatorIndex() {
        ...
        if (!isNullOrUndefined(this.parent.resourceBase) && (this.parent.activeViewOptions.group.resources.length > 0) &&
            ...
            let renderDates = (this.parent.uiStateValues.isGroupAdaptive) ?
    ...
    renderLeftIndent() {
        ...
        for (let i = 0; i < rowCount; i++) {
            ...
            let data = { className: [this.colLevels[i][0].className[0]], type: 'emptyCells' };
    ...
    getTimeSlotRows(handler) {
        ...
        let start = this.parent.globalize.parseDate(this.parent.workHours.start, { skeleton: 'Hm' });
        let end = this.parent.globalize.parseDate(this.parent.workHours.end, { skeleton: 'Hm' });
        for (let i = 0; i < length; i++) {
...
class Month extends ViewBase {
    ...
    createHeaderCell(td) {
        ...
        if (td.type === 'dateHeader') {
            ...
            if (this.parent.activeViewOptions.dateHeaderTemplate) {
                ...
                let title = this.parent.globalize.formatDate(td.date, { skeleton: 'full' });
    ...
    getMonthEnd(currentDate) {
        ...
        let lastWeekOfMonth = getWeekFirstDate(lastDateOfMonth(endDate), this.parent.firstDayOfWeek);
        ...
        if (this.parent.isAdaptive || isNullOrUndefined(this.parent.activeViewOptions.dateFormat)) {
            if (this.parent.activeViewOptions.interval > 1) {
                ...
                if (this.parent.selectedDate.getFullYear() === endDate.getFullYear()) {
                    let monthNames = (this.parent.globalize.formatDate(this.parent.selectedDate, { format: 'MMMM' }))
...
class TimelineEvent extends MonthEvent {
    ...
    renderEvents(event, resIndex) {
        ...
        let wrapIndex = this.parent.uiStateValues.isGroupAdaptive ? 0 : resIndex;
        ...
        if (startTime <= endTime) {
            ...
            this.renderedEvents.push(extend({}, event, null, true));
            let cellTd = this.appContainers[wrapIndex];
    ...
    getNormalEventsWidth(startDate, endDate, diffInDays) {
    ...
    getAppointmentLeft(schedule, startTime, endTime, day) {
    ...
    getPosition(schedule, startTime, endTime, day) {
    ...
    getRowTop(resIndex) {
        if (this.parent.activeViewOptions.group.resources.length > 0 && !this.parent.uiStateValues.isGroupAdaptive) {
            let tr = this.parent.element.querySelector('.' + CONTENT_WRAP_CLASS +
...
class TimelineViews extends VerticalView {
    ...
    getResourceTdData(i, tdData) {
        let resLevel = this.parent.resourceBase.lastResourceLevel[i];
...
class TimelineMonth extends Month {
    ...
    getContentSlots() {
        ...
        for (let row = 0; row < this.getRowCount(); row++) {
            for (let data of this.colLevels[this.colLevels.length - 1]) {
                ...
                if (this.parent.activeViewOptions.group.resources.length > 0 && !this.parent.uiStateValues.isGroupAdaptive) {
                    let resLevel = this.parent.resourceBase.lastResourceLevel[row];
    ...
    getMonthStart(currentDate) {
        let monthStart = firstDateOfMonth(resetTime(currentDate));
